...
namespace PSMDesktopApp.ViewModels
{
    public class EditServiceViewModel : Screen
    {
        ...
        public double TotalBiaya => Biaya - (Biaya * ((double)Diskon / 100)) + TambahanBiaya;
        ...
        public bool CanSave => !string.IsNullOrWhiteSpace(NamaPelanggan) && !string.IsNullOrWhiteSpace(TipeHp) && !string.IsNullOrWhiteSpace(Kerusakan) && Biaya > 0;
        ...
        public async Task<bool> UpdateService()
        {
            if (Diskon > 100)
            {
                DXMessageBox.Show("Diskon tidak boleh lebih dari 100%", "Edit servisan");
                return false;
            }
            else if (Diskon < 0)
            {
                DXMessageBox.Show("Diskon tidak boleh negatif", "Edit servisan");
            }
